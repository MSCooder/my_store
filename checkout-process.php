...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($shipping_address) && !empty($phone)) {
        try {
            ...
            // i. Main Orders table mein entry insert karein
            $stmt = $pdo->prepare("INSERT INTO orders (user_id, total_amount, shipping_address, phone, payment_method, status, created_at) VALUES (?, ?, ?, ?, ?, 'Pending', NOW())");
            $stmt->execute([$user_id, $subtotal, $shipping_address, $phone, $payment_method]);
            $order_id = $pdo->lastInsertId();

            // ii. Order items table mein har product ki entry details insert karein
            $stmtItems = $pdo->prepare("INSERT INTO order_items (order_id, product_id, product_name, price, qty, metal) VALUES (?, ?, ?, ?, ?, ?)");
            ...
            // iii. Order successful hone par cart khali kar dein
            unset($_SESSION['cart']);
...
<style>
    ...
    .form-group input, .form-group textarea, .form-group select { width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 4px; outline: none; font-size: 14px; }
    ...
    .btn-place-order { width: 100%; padding: 15px; background: #c4a47c; color: white; border: none; text-transform: uppercase; font-weight: 600; cursor: pointer; border-radius: 4px; transition: 0.3s; margin-top: 20px; }
    ...
    <div class="checkout-summary-box">
        ...
        <?php foreach($_SESSION['cart'] as $item): ?>
            <div class="summary-item">
                <span><?= htmlspecialchars($item['name']) ?> <strong>x <?= $item['qty'] ?></strong></span>
